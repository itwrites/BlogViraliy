...
- **Optional Base Path Support**: Enables reverse proxy deployments where the blog runs under a subdirectory (e.g., WordPress at ax.com with Blog Virality proxied at ax.com/blog). Configured per-site with automatic normalization (strips trailing slashes, ensures leading slash). Production builds automatically rewrite asset paths in HTML to include the base path prefix. **Important**: When using reverse proxy with a base path, the proxy must also forward `/assets/*` requests to the blog domain for dynamic chunk loading to work correctly.
