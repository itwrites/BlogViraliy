...
export async function generateAIPost(masterPrompt: string, keyword: string): Promise<{ title: string; content: string; tags: string[] }> {
  ...
  return {
    title: result.title || `Post about ${keyword}`,
    content: result.content || "",
    tags: Array.isArray(result.tags) ? result.tags : [keyword],
  };
}

export async function rewriteArticle(originalContent: string, originalTitle: string): Promise<{ title: string; content: string; tags: string[] }> {
