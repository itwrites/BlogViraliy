    ...
    <PublicThemeProvider settings={site.templateSettings}>
      ...
      <div className="min-h-screen bg-background text-foreground">
        <header className={`border-b bg-card ${templateClasses.isHeaderSticky ? 'sticky top-0 z-50' : ''}`}>
          <div className={`${templateClasses.contentWidth} mx-auto px-6 py-8`}>
        ...
        <main className={`${templateClasses.contentWidth} mx-auto px-6 py-16`}>
          {isLoading ? (
            ...
            <div className="grid gap-12 md:grid-cols-2">
              {posts.map((post, index) => (
                ...
                >
                  <div className={`${index === 0 && templateClasses.showHero ? 'aspect-[21/9]' : 'aspect-[4/3]'} bg-muted relative`}>
                  ...
                  <CardContent className={`${index === 0 && templateClasses.showHero ? 'p-10' : 'p-6'}`}>
                    ...
                    <h3 className={`${index === 0 && templateClasses.showHero ? 'text-3xl' : 'text-2xl'} font-light mb-3 line-clamp-2`} style={{ fontFamily: "var(--public-heading-font)" }} data-testid={`text-post-title-${post.id}`}>
                      {post.title}
                    </h3>
